...
router.post('/', function(req,res,next){
    ...
    connection.query('select userEmail,authNumber from auth where userEmail = ?',userEmail,function(err,results){ // 입력한 이메일에 대한 인증번호 검색

        if(err) return res.status(500).end();

        if(results.length == 0) { // 인증번호가 없다면 인증번호 메일 발송

            transporter.sendMail(mailOptions, function(err, info){
                if (!err){ // 인증번호 메일 발송 성공 시 데이터베이스에 저장
